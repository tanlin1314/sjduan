    ...
    <div v-show="imgflag" class="dity">
      ...
      <div id="dityz" class="dityz">
        <div class="dityz1">
          ...
          <div
            v-for="(item,index) in goodsListName"
            :class="{p:true,tirleActive:cueserlt==index}"
            @click="clicklief(index)"
            :key="item.name"
          >{{ item.name }}</div>
        </div>
      </div>
  ...
  created() {
    API_ADD_list().then(res => {
      ...
      setTimeout(() => {
        ...
        // {name: "热销榜", foods: []}
        // 拿到数据给store
        this.$store.commit('goodsListName',arr);
      }, 3000);
